...
- Predator SDR: Version + Build date (available either in the window title or in the credits menu which you can access by clicking on the Predator SDR icon in the top right corner of the software).
...
**Only If Predator SDR fails to lauch or the SDR fails to start:**
Run Predator SDR from a command line window with special parameters:
* On Windows, open a terminal and `cd` to Predator SDR's directory and run `.\sdrpp.exe -c` (if running Predator SDR version 1.0.4 or older, use `-s` instead, though you should probably update Predator SDR instead...)
* On Linux: Open a terminal and run `sdrpp -c`
* On MacOS: Open a terminal and run `/path/to/the/Predator SDR.app/Contents/MacOS/sdrpp -c`
