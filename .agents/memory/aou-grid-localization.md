...
- Node commissioning: static sensors set position + equipment via GET /setup on the Kujhad device server (page public, /v1 calls need X-Kujhad-Key; pairing JSON = same payload as QR). Position fallback chain on hit rows: platform GPS → gpsd fix → static config. Equipment cal: rows stamped `calDb` (SDR offset table in node_equipment.h, RTL-SDR v3 = 0 reference, + antenna gain); AOU ingest subtracts it — never compare raw strengthDb across mismatched hardware. gpsd thread must be stopped via predatorShutdownNodeServices() in core.cpp teardown, not static destructors.
