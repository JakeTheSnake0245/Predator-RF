...
private:
    static void menuHandler(void* ctx) {
        ...
        if (ImGui::InputTextIME(CONCAT("##_rigctl_srv_host_", _this->name), _this->hostname, 1023)) {
            config.acquire();
        ...
        if (ImGui::InputIntIME(CONCAT("##_rigctl_srv_port_", _this->name), &_this->port, 0, 0)) {
            config.acquire();
        ...
        if (parts[0] == "F" || parts[0] == "\\set_freq") {
            ...
            resp =
                ...
                */
                ...
                "0 0 0 0 0 0 0\n"
                /* End of TX frequency ranges. The Predator RF is receiver only. */
                "0 0 0 0 0 0 0\n"
