...
/* Predator: gate external desktop deps behind PREDATOR_BUILD.
 * On Android we vendor mbelib but skip pulseaudio/sndfile/ncurses/rtl-sdr/OSS.
 * The SDRPP module wrapper feeds samples in and pulls voice out via
 * predator_dsdfme_bridge.h ring buffers, so none of those layers are needed.
 * We provide opaque type stubs so structs that hold pointers to these still compile. */
#ifdef PREDATOR_BUILD
  #include <sys/types.h>      /* for ssize_t and size_t in stub prototypes */
  #include <stddef.h>

  /* Opaque pa_simple — only accessed via pointer. */
  typedef struct pa_simple   pa_simple;

  /* pa_sample_spec is declared as a value type in dsd_audio.c, so we need
   * a complete struct. The fields here mirror the real PulseAudio header. */
  typedef struct {
      int      format;
      uint32_t rate;
      uint8_t  channels;
  ...
  /* pa_buffer_attr is also declared as a value type in dsd_audio.c. */
  typedef struct {
      uint32_t maxlength;
      uint32_t tlength;
      uint32_t prebuf;
  ...
  /* pa_channel_map is opaque — only used as a NULL pointer. */
  typedef struct pa_channel_map pa_channel_map;

  /* PulseAudio sample-format enum constants. */
  #ifndef PA_SAMPLE_S16NE
  #  define PA_SAMPLE_S16NE      3
  #endif
  #ifndef PA_SAMPLE_FLOAT32NE
  #  define PA_SAMPLE_FLOAT32NE  5
  #endif
  #ifndef PA_STREAM_PLAYBACK
  #  define PA_STREAM_PLAYBACK   1
  #endif
  #ifndef PA_STREAM_RECORD
  #  define PA_STREAM_RECORD     2
  #endif

  /* OSS soundcard ioctl constants — code paths never run on Android, but the
   * literal symbols must still resolve at compile time. */
  ...
  /* SF_INFO is also declared as a value type in dsd_file.c / dsd_symbol.c.
   * Provide the fields referenced by the vendored code. */
  ...
  /* sndfile format flags used as bare identifiers throughout dsd-fme. */
  #ifndef SF_FORMAT_WAV
  #  define SF_FORMAT_WAV       0x010000
  #endif
  #ifndef SF_FORMAT_RAW
  #  define SF_FORMAT_RAW       0x020000
  #endif
  ...
  /* Prototypes for the stub layer (predator_dsdfme_stubs.c). Without these,
   * implicit-declaration warnings escalate to errors under -Werror. */
  ...
  long     sf_read_short   (SNDFILE *s, short *p, long n);
  long     sf_write_short  (SNDFILE *s, const short *p, long n);
  void     sf_write_sync   (SNDFILE *s);
  const char *sf_strerror  (SNDFILE *s);

  /* Make every ncurses entry point a no-op. WINDOW becomes void so that
   * struct fields of type WINDOW* keep their pointer size. */
  #define WINDOW void
  ...
  #define wprintw         (void)
  #define mvwprintw       (void)
  #define waddstr(w, s)   ((void)(w), (void)(s))
  #define wattron(w, a)   ((void)(w), (void)(a))
  #define wattroff(w, a)  ((void)(w), (void)(a))
  #define keypad(w, b)    ((void)(w), (void)(b), 0)
  #define noecho()        (0)
  #define cbreak()        (0)
  #define wgetch(w)       ((void)(w), -1)
  #define LINES           24
  #define COLS            80
...
#if defined(USE_RTLSDR) && !defined(PREDATOR_BUILD)
#include <rtl-sdr.h>
#endif
