# Predator SDR, The bloat-free SDR software<br>

![Screenshot](https://i.imgur.com/Ter2MQJ.png)
Predator SDR is a cross-platform and open source SDR software with the aim of being bloat free and simple to use.

![Build](https://github.com/AlexandreRouma/PredatorSDR/workflows/Build%20Binaries/badge.svg)
...
Nightly builds contain the very latest features and bugfixes. They are usually just stable as [normal releases](https://github.com/AlexandreRouma/PredatorSDR/releases) but are available basically minutes to hours after a change has been pushed to the code.
...
Download the latest release from [the Releases page](https://github.com/AlexandreRouma/PredatorSDR/releases) and extract to the directory of your choice.
...
There are currently no existing packages for other distributions, for these systems you'll have to [build from source](https://github.com/AlexandreRouma/PredatorSDR#building-on-linux--bsd).
...
There are currently no BSD packages, refer to [Building on Linux / BSD](https://github.com/AlexandreRouma/PredatorSDR#building-on-linux--bsd) for instructions on building from source.
...
### Run Predator SDR from the command line
...
## Installing Predator SDR

If you choose to run Predator SDR for development, you do not need this step.
...
If you wish to install Predator SDR, skip to the next step

First run Predator SDR from the build directory to generate a default config file
...
To install Predator SDR, run the following command in your ``build`` folder:
...
You will need a few special cmake argument on top of the linux ones. You will need to enable the portaudio sink modules `-DOPT_BUILD_PORTAUDIO_SINK=ON -DOPT_BUILD_NEW_PORTAUDIO_SINK=ON` and disable the usual rtaudio sink `-DOPT_BUILD_AUDIO_SINK=OFF` as well as the option to tell Predator SDR that it will run as a MacOS bundle `-DUSE_BUNDLE_DEFAULTS=ON`. On MacOS versions older than Catalina (10.15), you will also need to use the internal std::filesystem as the OS can't provide it `-DOPT_OVERRIDE_STD_FILESYSTEM=ON`.

Here is an example of build commands that will build almost all modules at the time of writing. You can always check the CI scripts for the latest arguments just in case but this should work. From the top of the PredatorSDR directory:
...
From the top of the PredatorSDR directory:

```sh
sh make_macos_bundle.sh ./build ./Predator SDR.app
```

This will create a `Predator SDR.app` bundle that you can instal like any other MacOS app by dragging it into Applications.
...
Modules in beta are still included in releases for the most part but not enabled in Predator SDR (need to be instantiated).

## Sources

| Name                 | Stage      | Dependencies      | Option                         | Built by default| Built in Release        | Enabled in Predator SDR by default |
|----------------------|------------|-------------------|--------------------------------|:---------------:|:-----------------------:|:---------------------------:|
...
## Predator SDR crashes then it won't start again no matter what

This is a bug in 1.0.0 that was fixed in 1.0.1

In some cases, if a crash happened while the config was being saved, the config file would be corrupted and Predator SDR would refuse to start because of it.
...
If the module was included in a later update, it's not enabled in the config. The easiest way to fix this is just to delete the `config.json` file and let Predator SDR recreate it (you will lose your setting relating to the main UI like VFO colors, zoom level and theme).
...
The solution until a fixed libhackrf version is released is to disable the soapy_source module from Predator SDR. For this, go into the "Module Manager" menu and click the `-` button next to the row with "soapy_source". After that, restart Predator SDR.
