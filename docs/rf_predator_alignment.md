...
- Decoder Bridges shell: receive-only configuration scaffolding for P25 (Phase 1+2), RTL433 ISM, POCSAG/FLEX paging, ADS-B aircraft, and AIS marine. Host/port/mode/notes persisted to `predatorDecoderBridges`; live status indicators in the Network tree mark which protocols have an active bridge feeding them.
- RTL433 native ingestion: a dedicated worker thread (`predator::Rtl433Ingester` in `core/src/predator/decoder_ingest.h`) opens either a TCP client connection or a UDP server bind based on the persisted bridge mode, parses newline-delimited JSON emitted by the standard `rtl_433` companion process, and pushes normalized events (model → networkId, channel/subtype → talkgroup, id → radioId, freq MHz → frequencyHz, rssi → strengthDb) onto a thread-safe queue that the UI thread drains each frame and folds into `predatorEvents`. The thread is reconciled to bridge config every frame, reconnects with exponential backoff (500 ms → 8 s) on disconnect, and exposes live connection status + cumulative event count in the Network → Decoder Bridges section.
