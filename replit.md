...
- Android soft-keyboard text input is captured by a **4×4 px, alpha=0.01** `EditText` overlay (`imeCaptureView` in `MainActivity.kt`), NOT by `dispatchKeyEvent`. Modern IMEs (Gboard / SwiftKey / Samsung) commit letters via `InputConnection.commitText()` and never emit hardware `KeyEvent`s for letters. The TextWatcher walks inserted text by **Unicode code point** (`Character.codePointAt` + `charCount`, NOT `s[i].code`) into `unicodeCharacterQueue` for `PollUnicodeChars()` to drain. Backspace bridges via `KEYCODE_DEL` → `0x08` AND via the `(before>0, count==0)` TextWatcher branch (Gboard does NOT emit `KEYCODE_DEL` — it calls `deleteSurroundingText`). NativeActivity's `NativeContentView` aggressively reclaims focus, so `showSoftInput()` must `bringToFront` → `decorView.clearFocus` → `requestFocus` → `imm.restartInput` → `showSoftInput(SHOW_FORCED)` AND post a one-frame-later focus reassert. Diagnostic via `adb logcat -s PredatorRF` — look for `imeCaptureView.requestFocus() returned false` (smoking gun if typing is dropped). EditText alpha must be 0.01 (NOT 0.0) and size 4×4 (NOT 1×1) — IMEs reject fully invisible / zero-size views.
