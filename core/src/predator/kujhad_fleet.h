...
"<div class=hint>Net RSSI correction at that frequency: <span id=cal>0.0</span> dB (SDR offset + interpolated antenna gain).</div>"
...
"let sdrOptions=[];let presets=[];"
// Antenna curve editor: dynamic rows of (MHz, dB) points. Mirrors the
// C++ math: log10(f) interpolation, ends held, empty curve = 0 dB.
